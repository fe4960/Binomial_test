...
my $pt_freq = $ARGV[0]; # ac/(number of pt *2)
 $pt_freq = $pt_freq *2;
my $F_factor = 0.1;
my $output = "HGMD/data/power_test_pf_dom_$ARGV[0]"."_test_new_ground_truth_new_spec_inbreeding_$F_factor"."_new1";
my $output1 = "HGMD/data/power_test_pf_dom_$ARGV[0]"."_test_new_ground_truth_new_value_spec_inbreeding_$F_factor"."_new1";
my $R_comd = "Software/R-3.5.0/bin/R";
my $dis_freq = 0.001;
open(OUTPUT,">$output");
...
my @normal_freq = (0.000005,0.00001,0.00002,0.00005);
	...
	for my $i (@normal_freq){  # allele freq
		 ...
		 print OUTPUT1 "$i\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $n=0;
                   my $normal_freq = $i; 
                   my $pt_freq_exp = $normal_freq / (1-sqrt(1-$dis_freq)); # * (1+$F_factor); 
...
sub bin_test {
my $R_script = "STGD/scripts/R_pt_AD_spec_inbr$ARGV[0].R";
my $R_out = "STGD/scripts/R_pt_AD_spec_inbr$ARGV[0].out";
