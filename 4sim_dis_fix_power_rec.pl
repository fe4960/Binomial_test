...
my $main = "/home";
my $pt_freq = $ARGV[0];
my $output = "HGMD/data/power_test_df_$ARGV[0]"."_test_new_ground_truth_new";
my $output1 = "HGMD/data/power_test_df_$ARGV[0]"."_test_new_ground_truth_new_value";
my $normal_freq = 0.0002;
open(OUTPUT,">$output");
...
my @dis_freq = (0.0001, 0.00007, 0.00005,0.00001);
	...
	for my $i (@dis_freq){  # allele freq
		 ...
		 print OUTPUT1 "$i\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $pt_freq_exp = $normal_freq / sqrt($dis_freq); 
...
sub bin_test {
my $R_script = "STGD/scripts/R_dis_AR$ARGV[0].R";
my $R_out = "STGD/scripts/R_dis_AR$ARGV[0].out";
