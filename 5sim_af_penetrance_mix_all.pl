...
#my @normal_freq = (0.5*(1-sqrt(1-$dom_dis_freq))/3.5/0.2, (1-sqrt(1-$dom_dis_freq))/3.5/0.5, (1-sqrt(1-$dom_dis_freq))/3.5/0.8,(1-sqrt(1-$dom_dis_freq))/3.5);
my @normal_freq = (0.5*(1-sqrt(1-$dom_dis_freq))/3.5/0.2/5, (1-sqrt(1-$dom_dis_freq))/3.5/0.5/5, (1-sqrt(1-$dom_dis_freq))/3.5/0.8/5,(1-sqrt(1-$dom_dis_freq))/3.5/5);

print "@normal_freq\n";
my @freq_factor = (0.2, 0.5, 0.8, 1);
my $label ="conv";
...
my $output1 = "HGMD/data/power_test_af_penet_dom_test_new_ground_truth_new_value_mix"."_$label";
...
#	for my $freq_factor (@freq_factor){  # allele freq
           for(my $i=0; $i<=$#normal_freq; $i++){
		 print OUTPUT1 "$freq_factor[$i]\t$normal_freq[$i]\t";

             for my $s (@size){   # pt cohort aize
                   my $size = $s;
                   
                   my $power = binProb($size,$normal_freq[$i],$dom_dis_freq,"dom_power",$freq_factor[$i]);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
$output1 = "HGMD/data/spec_test_af_penet_dom_test_new_ground_truth_new_value_mix"."_$label";
open(OUTPUT1,">$output1");
print OUTPUT1 "AF\t";
...
#		 print OUTPUT1 "$freq_factor\t";
		 ...
		 print OUTPUT1 "$freq_factor[$i]\t$normal_freq[$i]\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq[$i],$dom_dis_freq,"dom_spec",$freq_factor[$i]);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
#$normal_freq = 0.0005;
#$normal_freq = 0.001;
#@normal_freq = (0.5*sqrt($rec_dis_freq)/3.5/0.2, sqrt($rec_dis_freq)/3.5/0.5, sqrt($rec_dis_freq)/3.5/0.8, sqrt($rec_dis_freq)/3.5);
@normal_freq = (0.5*sqrt($rec_dis_freq)/3.5/0.2/5, sqrt($rec_dis_freq)/3.5/0.5/5, sqrt($rec_dis_freq)/3.5/0.8/5, sqrt($rec_dis_freq)/3.5/5);

print "@normal_freq\n";

$output1 = "HGMD/data/power_test_af_penet_rec_test_new_ground_truth_new_value_mix"."_$label";
open(OUTPUT1,">$output1");
...
#		 print OUTPUT1 "$freq_factor\t";
		 ...
		 print OUTPUT1 "$freq_factor[$i]\t$normal_freq[$i]\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq[$i],$rec_dis_freq,"rec_power",$freq_factor[$i]);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
$output1 = "HGMD/data/spec_test_af_penet_rec_test_new_ground_truth_new_value_mix"."_$label";
open(OUTPUT1,">$output1");
print OUTPUT1 "AF\t";
...
#		 print OUTPUT1 "$freq_factor\t";
		 ...
		 print OUTPUT1 "$freq_factor[$i]\t$normal_freq[$i]\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq[$i],$rec_dis_freq,"rec_spec",$freq_factor[$i]);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
#if($_[3] eq "dom_power"){
#print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=($_[1]*2 + $_[1]/\(1-sqrt\(1-$_[2]\)\))/2\)<=0.05,1,0)\n";
#}elsif($_[3] eq "dom_spec"){
#print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=($_[1]*2 + $_[1]/\(1-sqrt\(1-$_[2]\)\))/2\)>0.05,1,0)\n";
#}elsif($_[3] eq "rec_power"){
#print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=($_[1]+$_[1]/sqrt\($_[2]\))/2\)<=0.05,1,0)\n";
#}elsif($_[3] eq "rec_spec"){
#print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=($_[1]+$_[1]/sqrt\($_[2]\))/2\)>0.05,1,0)\n";
#}
