...
#my @dis_factor = (0.1, 0.2, 0.5,0.8, 1,2,5,10);
my @dis_factor = (0.2, 0.5,0.8, 1,2,5,10,20);
#my @dis_factor = (0.1,0.2, 0.5,0.8, 1,2,5,10,20);
my $label ="conv_string";
my @size = (50,80,100,120,200,250,300,400,500,600,700,800,900,1000);
my $normal_freq = 0.00005;
#my $normal_freq = 0.0001;


my $output1 = "HGMD/data/power_test_dis_mis_dom_test_new_ground_truth_new_value_$normal_freq"."_$label";
...
print OUTPUT1 "AF\t";
	...
	for my $dis_factor (@dis_factor){  # allele freq
		 print OUTPUT1 "$dis_factor\t";

             for my $s (@size){   # pt cohort aize
                   my $size = $s;
                   
                   my $power = binProb($size,$normal_freq,$dom_dis_freq,"dom_power",$dis_factor);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
$output1 = "HGMD/data/spec_test_dis_mis_dom_test_new_ground_truth_new_value_$normal_freq"."_$label";
open(OUTPUT1,">$output1");
print OUTPUT1 "AF\t";
	...
	for my $dis_factor (@dis_factor){  # allele freq
		 print OUTPUT1 "$dis_factor\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq,$dom_dis_freq,"dom_spec",$dis_factor);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
$output1 = "HGMD/data/power_test_dis_mis_rec_test_new_ground_truth_new_value_$normal_freq"."_$label";
open(OUTPUT1,">$output1");
	...
	for my $dis_factor (@dis_factor){  # allele freq
		 print OUTPUT1 "$dis_factor\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq,$rec_dis_freq,"rec_power",$dis_factor);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
$output1 = "HGMD/data/spec_test_dis_mis_rec_test_new_ground_truth_new_value_$normal_freq"."_$label";
open(OUTPUT1,">$output1");
print OUTPUT1 "AF\t";
	...
	for my $dis_factor (@dis_factor){  # allele freq
		 print OUTPUT1 "$dis_factor\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $power = binProb($size,$normal_freq,$rec_dis_freq,"rec_spec",$dis_factor);
                   print OUTPUT1 "$power\t";
               }
                   print OUTPUT1 "\n";
         }
...
if($_[3] eq "dom_power"){
print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=$_[1]/\(1-sqrt\(1-$_[2]\*$_[4]\)\)\)<=0.05,1,0)\n";
}elsif($_[3] eq "dom_spec"){
print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=$_[1]/\(1-sqrt\(1-$_[2]\*$_[4]\)\)\)>0.05,1,0)\n";
}elsif($_[3] eq "rec_power"){
print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=$_[1]/sqrt\($_[2]\*$_[4]\)\)<=0.05,1,0)\n";
}elsif($_[3] eq "rec_spec"){
print OUTFILE "factor=ifelse(pbinom\(b, size=$_[0], prob=$_[1]/sqrt\($_[2]\*$_[4]\)\)>0.05,1,0)\n";
}
