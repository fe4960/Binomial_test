...
my $pt_freq = $ARGV[0];;
 $pt_freq = $pt_freq *2;
my $output = "HGMD/data/power_test_pf_dom_$ARGV[0]"."_test_new_ground_truth_new_inbreeding_$F_factor"."_new1";
my $output1 = "HGMD/data/power_test_pf_dom_$ARGV[0]"."_test_new_ground_truth_new_value_inbreeding_$F_factor"."_new1";
	...
	for my $i (@normal_freq){  # allele freq
		 ...
		 print OUTPUT1 "$i\t";
             ...
             for my $s (@size){   # pt cohort aize
                   ...
                   my $n=0;
                   my $obser_exp = int($pt_freq*(1+$F_factor)*$size);
